...
describe('BoardOperations', () => {
  ...
  describe('slide', () => {
    test('should slide non-zero values to the left', () => {
      const input = [0, 2, 0, 4];
      const expected = [2, 4, 0, 0];
      expect(slide(input)).toEqual(expected);
    });

    test('should preserve order of non-zero values', () => {
      const input = [0, 2, 0, 4, 0, 8];
      const expected = [2, 4, 8, 0, 0, 0];
      expect(slide(input)).toEqual(expected);
    });
