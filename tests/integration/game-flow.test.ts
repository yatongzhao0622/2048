import { createGame, move, getScore, checkIfGameOver, defaultConfig } from '../../src/core/game';
        ...
        it('should generate new tiles with expected distribution', () => {
            ...
            for (let i = 0; i < 100; i++) {
                ...
                if (currentGame.board.some((row, r) => 
                    row.some((cell, c) => cell !== prevBoard[r][c])
                )) {
                    const newTile = currentGame.board.flat().find((val, idx) => 
                        val !== 0 && prevBoard.flat()[idx] === 0
                    );
                    if (newTile) newTileValues.push(newTile);
                }
            }
